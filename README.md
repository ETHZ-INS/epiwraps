...
the respective *[Gviz](https://bioconductor.org/packages/3.13/Gviz)* function can be passed through the
`genes.params` (for \code{\link[Gviz](GeneRegionTrack)}), `align.params` (for 
\code{\link[Gviz](AlignmentsTrack)}, when plotting individual reads), or 
`tracks.params` (for any other \code{\link[Gviz](DataTrack)}).

In addition, in addition to passing filepaths or `GRanges`, any 
*[Gviz](https://bioconductor.org/packages/3.13/Gviz)* track(s) can be passed (i.e. objects inheriting 
the `GdObject` class) can be passed, enabled full track customization when 
needed.
