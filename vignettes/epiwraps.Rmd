...
The `bam2bw` function can be used to create a bigwig file from a bam file:

```{r}
bam2bw(bamfile=bam, output_bw="test.bw", binWidth=10, scaling=TRUE)
```

The <a href="bam2bw.html">bam2bw vignette</a> explains some of the basic concepts
being the different ways to compute enrichment along the genome and main options
of the function (see also `?bam2bw` for more detail).

## Peak calling

A peak calling function can be used, either against an input control or against local or global backgrounds:

```{r, eval=FALSE}
p <- callPeaks(bam, fragLength=50)
```

Note that this function is still under heavy development!
