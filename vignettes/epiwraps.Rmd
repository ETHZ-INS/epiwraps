---
title: "epiwraps: easy wrappers for easy epigenomic data visualization"
author: 
...
bwf2 <- system.file("extdata/example_rna.bw", package="epiwraps")
plotSignalTracks(list(RNA=bwf2), region="8:22165140-22212326", genomeAxis=TRUE)
# we could plot multiple tracks as follows:
plotSignalTracks(list(track1=bwf2, track2=bwf2), region="8:22165140-22212326")
```
...
plotSignalTracks(list(RNA=bwf2, regions=myregions), region="8:22165140-22212326")
```
...
```{r, fig.width=6, fig.height=2}
myregions <- GRanges("8", IRanges(c(22166000,22202300), width=3000))
plotSignalTracks(list(RNA=bwf2, regions=myregions), colors=c("red", "black"),
...
```{r, fig.width=6, fig.height=2.5}
plotSignalTracks(list(track1=bwf2, combined=c(bwf2,bwf2)),
                 region="8:22165140-22212326")
```
...
plotSignalTracks(c(coverage=bwf2), region="BMP1", ensdb=ensdb, 
                 transcripts="full")
```
...
`r BiocStyle::Biocpkg("Gviz")` track(s) can be passed (i.e. objects inheriting 
...
In addition, signal matrices can be combined either manually, or using the 
...
advisable to use minimal trimming (e.g. >0.99 -- the default is 0.998).
...
```{r, fig.width=4, fig.height=3}
ggplot(d, aes(position, mean, colour=sample)) + geom_line(lwd=2)
```
