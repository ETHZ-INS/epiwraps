...
abstract: |
  This vignette document the use of the 'bam2bw' function of the epiwraps 
...
<a href="epiwraps.html">the vignette</a>), represent chromatin accessibility 
(ATAC-seq) signals around CTCF motifs. The different signals are based on 
different bigwig files derived from the same bam file using the functions 
described above.

```{r egBwigs, echo=FALSE}
knitr::include_graphics(system.file('docs', 'example_bigwigs_hm.png', 
                                    package='epiwraps'))
```
...
* The second heatmap shows where the transposase inserted itself, and was 
generated with `type="ends"` (to be precise, a +4/-5 shift could also be 
included with the `shift` argument). Using this we can see a nice footprint 
protected from the transposase by CTCF binding.
* The third heatmap was generated with `maxFragLength=115`, which aims to 
capture fragments that are too small to be protected by a nucleosome, and 
therefore must be bound by a transcription factor. We see that this signal is 
more concentrated right on the center (i.e. the CTCF sites).
* The fourth heatmap combines the previous two.
* The last heatmap shows the centers of nucleosome-sized fragments using
`type="center", minFragLength=150, maxFragLength=220`. Well-positioned 
nucleosomes are thus highlighted on each sides of the CTCF bindings.
