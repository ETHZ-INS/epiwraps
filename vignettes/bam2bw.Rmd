...
```{r plotGR, fig.height=1, fig.width=4}
...
```{r plotBw1, fig.width=4.5, fig.height=2.5}
...
```{r plotBw2, fig.width=4.5, fig.height=3.5}
# Using mean per bin:
...
```{r plotBwExtended, fig.width=4.5, fig.height=2.5}
...
```{r plotBwEnds, fig.width=4.5, fig.height=3.5}
# Here the reads are 50bp, and we want to extend them to 100bp, hence _by_ 50:
...
was used with `maxFragLength=120`. These are indicative of TF
...
generated with 
`trim=4L, binWidth=1L, maxFragLength=120, type="ends"`. Using this 
we can see a nice footprint protected from the transposase by CTCF binding.

For more information about these parameters, see `?bam2bw`.
